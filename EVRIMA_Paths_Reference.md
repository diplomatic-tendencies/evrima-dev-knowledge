...
│   │               ├── enabled.txt                # Optional per-mod force-enable marker (OVERRIDES mods.txt)
│   │               ├── dlls/
...
There is a second, easily-missed enable mechanism: an `enabled.txt` file placed **inside an individual mod's own folder** (`Mods/<ModName>/enabled.txt`). It is not a Mods-root alternative to `mods.txt` and UE4SS does not auto-generate it — it is an optional marker whose presence **force-enables that one mod regardless of its `mods.txt` line**. A stray `enabled.txt` overrides `mods.txt : 0`, so a mod you "disabled" in `mods.txt` will keep loading if that file is still in its folder. This is a real gotcha: to truly disable a mod, set it to `0` in `mods.txt` **and** delete any `enabled.txt` in its folder. When in doubt, the boot log's mod-start lines are the ground truth for what actually loaded.
...
The `AdminsSteamIDs` list (on `TIGameStateBase`) is what the `SetAdminCred` heartbeat's bool parameter reflects. Adding a steam to this list and rebooting causes that player's `SetAdminCred` to fire with `bool=true`. Post-0.21.720 note: one production box had this list present and admin silently dead until the same block was *also* added under `[/Script/TheIsle.TIGameSession]`, while another runs fine with `TIGameStateBase` only — keep the block under both sections; see the migration doc's gotcha 3.
